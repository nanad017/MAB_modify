...
class Classifier:
    def __init__(self, classifier_name):
        ...
        elif classifier_name == 'custom_xgboost':
            logger_cla.info('Loading internal Custom XGBoost...')
            self.model = CustomXGBoostModel(
                '/root/Malware-Detection-System/artifacts/binary_xgboost.joblib',
                thresh=0.5,
            )
        elif classifier_name == 'remote_detector':
            logger_cla.info('Loading remote detector API...')
            self.model = RemoteDetectorModel(
                'http://172.17.0.1:8000/score',
                thresh=0.5,
                shared_root='/root/MAB-malware/data/share',
            )
        else:
